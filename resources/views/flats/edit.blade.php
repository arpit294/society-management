<form id="flat-ajax-form" method="POST" action="{{ route('flats.update', $flat->id) }}">
    ...
    <div class="modal-body">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6 {{ ($globalBillingMethod ?? 'fixed') === 'per_sqft' ? 'd-none' : '' }}" id="maintenance_rate_wrapper_edit">
                ...
                <select name="flat_type_id" id="flat_type_id_select_edit" class="form-select" {{ ($globalBillingMethod ?? 'fixed') === 'per_sqft' ? '' : 'required' }}>
                    ...
                    @foreach ($flatTypes as $type)
                        <option value="{{ $type->id }}" data-calc-method="{{ $type->calculation_method }}" data-category-type="{{ strtolower($type->category_type ?? 'residential') }}" data-name="{{ strtolower($type->name) }}" {{ old('flat_type_id', $flat->flat_type_id) == $type->id ? 'selected' : '' }}>
            ...
            <div class="col-md-6 {{ ($globalBillingMethod ?? 'fixed') !== 'per_sqft' && ($flat->flatType->calculation_method ?? 'fixed') === 'fixed' && !in_array(strtolower($flat->flatType->category_type ?? 'flat'), ['shop', 'office', 'showroom', 'warehouse']) ? 'd-none' : '' }}" id="area_sqft_wrapper_edit">
        ...
        <script>
            function filterMaintenanceRateOptionsEdit() {
                const unitType = document.getElementById('unit_type_select_edit')?.value || 'flat';
                ...
                if (typeof window.checkBillingMethodEdit === 'function') {
                    window.checkBillingMethodEdit();
                }
            }
            ...
            document.getElementById('unit_type_select_edit')?.addEventListener('change', function() {
                ...
                filterMaintenanceRateOptionsEdit();
            });
            ...
            (function() {
                ...
                window.checkBillingMethodEdit = function() {
                ...
                flatTypeSelect?.addEventListener('change', window.checkBillingMethodEdit);
                document.getElementById('unit_type_select_edit')?.addEventListener('change', window.checkBillingMethodEdit);
                filterMaintenanceRateOptionsEdit();
            })();
        </script>
    </div>
