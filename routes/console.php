...
Artisan::command('app:update-bill-amounts', function () {
    $bills = \App\Models\MaintenanceBill::with(['maintenance'])->where('status', '!=', 'paid')->get();
    $count = 0;

    $getSettingValues = function (string $type) {
        return [
            "apply_{$type}" => \App\Models\Setting::get("apply_{$type}", '0'),
            'type' => \App\Models\Setting::get("{$type}_type", 'percentage'),
            'due_days' => \App\Models\Setting::get("{$type}_due_days", '15'),
    ...
    $penaltySettings = $getSettingValues('penalty');
    $discountSettings = $getSettingValues('discount');
    ...
    foreach ($bills as $bill) {
        ...
        if ($bill->maintenance && $bill->maintenance->month && $bill->maintenance->year) {
            try {
                $billDate = \Carbon\Carbon::parse('1 '.$bill->maintenance->month.' '.$bill->maintenance->year)->startOfMonth();
            } catch (\Exception $e) {
                $billDate = \Carbon\Carbon::parse($bill->generated_date)->startOfMonth();
            }
        } else {
            $billDate = \Carbon\Carbon::parse($bill->generated_date)->startOfMonth();
        }
        ...
        if ($penaltySettings['apply_penalty'] === '1' || $penaltySettings['apply_penalty'] === 'true') {
            $dueDate = \Carbon\Carbon::parse($bill->generated_date)->addDays($dueDays);
    ...
    \Illuminate\Support\Facades\Log::info("UpdateBillAmounts command ran. Updated {$count} bills.");
})->purpose('Daily update of penalty and discount amounts for unpaid bills based on settings');

Schedule::command('app:update-bill-amounts')->daily();
